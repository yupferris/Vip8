...
void Chip8::Reset()
{
	...
	timerDelay = 0;
	timerSound = 0;
	for (int i = 0; i < 4096; i++) ram[i] = Random::NextInt() & 255;
...
void Chip8::LoadRom(string fileName)
{
	try
	{
		...
		if (input.fail()) throw EXCEPTION("Could not load ROM image.");
		int size = (int) input.tellg();
		if (size > 3584) throw EXCEPTION("ROM image is too large.");
		romSize = size;
		if (rom) delete rom;
	...
	{
		MessageBox(0, e.GetMsg().c_str(), "Chip8 Error", MB_OK);
	}
}
...
void Chip8::Start()
{
	if (running) throw EXCEPTION("Emulator already running.");
	if (rom)
	{
...
void Chip8::Update()
{
	...
	try
	{
		...
		for (int i = 0; i < speed; i++)
		{
			...
			if (pc < 0 || pc >= 4095) throw EXCEPTION("PC out of bounds.");
